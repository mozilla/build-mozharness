...
class PandaTalosTest(TestingMixin, MercurialScript, BlobUploadMixin, MozpoolMixin, BuildbotMixin, SUTDeviceMozdeviceMixin):
    ...
    def preflight_talos(self, suite_category, suites):
        ...
        self.run_command(cmd, dirs['abs_preflight_talos_dir'], env=env, halt_on_failure=True, fatal_exit_code=suites.get('fatal_exit_code', 3))
    ...
    def download_and_extract(self):
        ...
        self._download_unzip(self.installer_url,
                             dirs['abs_fennec_dir'])
        # this is ugly but you can't specify a file in download_unzip to
        # extract the file to, by default it's the abs_work_dir
